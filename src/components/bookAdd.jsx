...
function BookAdd() {
			...
			(error) => {
				switch (error.code) {
					case "storage/unauthorized":
						toast.warn("User doesn't have permission to access the object", {
					...
					case "storage/canceled":
						toast.warn("User canceled the upload", {
							hideProgressBar: true,
							autoClose: 1000,
							theme: "dark",
						});
					...
					case "storage/unknown":
						toast.warn("Unknown error occurred, inspect error.serverResponse", {
							hideProgressBar: true,
							autoClose: 1000,
							theme: "dark",
						});
		...
		if (currentUser) {
			try {
				await addDoc(collection(db, `Library`), {
					...
					() =>
						progress === 100.0 &&
						toast.success("Book is Added to library!", {
							hideProgressBar: true,
							autoClose: 1500,
							theme: "dark",
						})
	...
	return (
		<div>
			<Form
				className='form-group library-container'
				...
				<div className='info'>
					...
					<Form.Group className='mb-3' controlId=''>
						<Form.Control
							type='text'
							value={autour}
							onChange={(e) => setAutour(e.target.value)}
