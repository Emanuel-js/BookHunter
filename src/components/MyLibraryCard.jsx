		...
		<div>
			<Card style={{ width: "20rem", margin: 10 }}>
				...
				<Card>
					...
					<Card.Text className='mr-l'>publisher: {books?.publisher}</Card.Text>
				...
				<Card.Body>
					<div className='sub-post'>
						<Card.Img
							variant='top'
							src={
								books.ownerImg ? books.ownerImg : "https://i.pravatar.cc/300"
							}
						...
						{currentUser.uid === books.UID ? (
							<div className=''>
								{" "}
								<Badge bg='secondary'>Your share this Book</Badge>
							</div>
						) : (
							<div>
								<Badge bg='secondary'>{books.ownerName} share this book</Badge>
							</div>
						)}
					</div>
				</Card.Body>
				<Card.Link
					...
					target='_blank'
					download
					className='btn-custom btn-prim link'
				>
					Read
				</Card.Link>
				{ismy && currentUser.uid === books.UID ? (
					<button
						className='btn btn-danger'
						onClick={(e) => deleteHandler(data?.id)}
					>
						delete
					</button>
				) : (
