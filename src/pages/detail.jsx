...
function Detail() {
	...
	useEffect(() => {
		const getrelated = async () => {
			await axios
				.get(`${url}reviews.json?author=${book.author}&api-key=${APIKEY}`)
				.then((d) => {
		...
		if (currentUser) {
			try {
				await setDoc(
					doc(
						db,
						`Reading/${currentUser.uid}`,
						"reading",
						`${
							book?.primary_isbn10 || book?.isbns[0].isbn10 || book?.isbn13[0]
						}`
					),
					{
						UID: currentUser.uid,
		...
		if (currentUser) {
			try {
				await setDoc(
					...
					toast.success("your Favorite Book is Added!", {
						hideProgressBar: true,
						autoClose: 1500,
						theme: "dark",
					})
				...
				<div className='detail-section'>
					...
					<div className='detail'>
						...
						<div className='sub-title'>
							{" "}
							by {book.author || book.book_author}🙋
						</div>
						...
						<div className='co1'>
							<div className='rete'>
								{published_date ? (
									<div>
										<b>publishedDate:</b> {published_date}📅
									</div>
								) : (
									""
								)}
							...
							<br />
							<div className='date'>
								<b>publisher:</b> {book.publisher || book.byline}
							</div>
						</div>
						<div className='btn-co1'>
							<button
								className='btn-custom btn-prim'
								onClick={(e) => addToRead(book)}
							>
								Add To Read
							</button>
							<button
								className='btn-custom btn-sec'
								onClick={(e) => AddToFav(book)}
							>
								Add to Fav
							</button>
						</div>
									...
									if (l.name === "Amazon")
										return (
											...
											>
												<img
													alt='logo'
													src='https://img.icons8.com/cute-clipart/64/000000/amazon.png'
												/>
											</a>
										);
										...
										return (
											...
											>
												<img
													alt='logo'
													src='https://img.icons8.com/color/48/000000/ibooks.png'
												/>
											</a>
										);
										...
										return (
											...
											>
												<img
													alt='logo'
													src='https://img.icons8.com/ios/50/000000/bookshop.png'
												/>
											</a>
										);
									else
										return (
											...
											>
												<img
													alt='logo'
													src='https://img.icons8.com/color-glass/48/000000/bookmark.png'
												/>
											</a>
										);
