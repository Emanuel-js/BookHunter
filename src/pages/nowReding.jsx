...
function NowReading() {
	...
	useEffect(() => {
		const getData = async () => {
			const querySnapshot = await getDocs(
				collection(db, `Reading/${currentUser?.uid}`, "reading")
			);
	...
	const deleteData = async (id) => {
		await deleteDoc(doc(db, `Reading/${currentUser.uid}`, "reading", id)).then(
			(res) => {
	...
	return (
		<div>
			...
			<Header />
			{book?.length > 0 && (
				<div className='title' style={{ textAlign: "center", margin: 30 }}>
					{" "}
					You have{book.length} {book.length === 1 ? "book" : "books"} to read📓
				</div>
			)}
			{book?.length !== 0 ? (
				book.map((book, id) => {
				...
				<div className='title center'>No Book To Read Is Here! 🍮</div>
			)}
			<Footer />
		</div>
	);
